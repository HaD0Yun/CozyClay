...
def _validate_marker_paths(
    ...
    expected_backup = (
        root / ".omb" / "transactions" / str(transaction_id) / "base.blend"
    )
    if backup != expected_backup:
        raise PreparedTransactionError(
            "base_backup_path must be .omb/transactions/<transaction_id>/base.blend"
        )
...
def _ensure_transaction_directories(root: Path, transaction_id: str) -> Path:
    omb = root / ".omb"
    _ensure_private_directory(omb, root)
    transactions = omb / "transactions"
...
def marker_path(project_root: str | os.PathLike[str]) -> Path:
    ...
    return _normalize_project_root(project_root) / ".omb" / "prepared-transaction.json"
...
def write_marker(
    ...
    omb = root / ".omb"
    _ensure_private_directory(omb, root)
    destination = omb / "prepared-transaction.json"
...
def read_marker(
    ...
    *,
    ...
    root = _normalize_project_root(project_root)
    path = root / ".omb" / "prepared-transaction.json"
    try:
...
def prepare_transaction(
    *,
    ...
    existing_path = root / ".omb" / "prepared-transaction.json"
...
def cleanup_transaction(
    ...
    *,
    ...
    marker_file = root / ".omb" / "prepared-transaction.json"
