...
def capture_viewport() -> dict:
    ...
    try:
        ...
        image = bpy.data.images.new("omb_viewport_capture", width, height, alpha=True)
        try:
            image.pixels.foreach_set(pixels.ravel())
            with tempfile.TemporaryDirectory(prefix="omb-viewport-") as directory:
...
def _encode_thumbnail_png(png_path: Path, width: int, height: int, method: str) -> dict:
    ...
    with tempfile.TemporaryDirectory(prefix="omb-viewport-thumb-") as directory:
