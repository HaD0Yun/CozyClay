...
RUNTIME_PRODUCER_ID = "omb-addon-runtime"
...
def durable_project_base(project_directory: object, project_id: str) -> tuple[str, str]:
    ...
    This is the same .omb/project.json source the bridge dispatch consults
    ...
    if project is None:
        raise EVIDENCE_PRODUCTION_FAILED(
            "no durable project exists in .omb/project.json"
        )
...
def _select_subject(scene):
    """Prefer an OMB-owned armature (character rig), else the largest mesh."""
    armatures = [
        scene_object
        for scene_object in scene.objects
        if scene_object.type == "ARMATURE"
        and isinstance(scene_object.get("omb.entity_id"), str)
    ]
    if armatures:
...
def produce_directing_evidence(
    ...
    *,
    ...
    if scene.get("omb.project_id") != project_id:
        raise EVIDENCE_PRODUCTION_FAILED(
