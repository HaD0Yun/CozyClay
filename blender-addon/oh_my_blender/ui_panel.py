...
def draw_panel(
    ...
    properties = getattr(getattr(context, "scene", None), "omb_panel_chat", None)
    ...
    if snapshot.can_submit:
        ...
        row.operator("omb.send_prompt", text="Send")
    if snapshot.can_cancel:
        row = layout.row(align=True) if callable(getattr(layout, "row", None)) else layout
        row.operator("omb.cancel_turn", text="Cancel")
    if active_controller is None or controller_state != ControllerState.ACTIVE:
        layout.operator("omb.reconnect_controller", text="Reconnect")
    return tuple(labels)
