...
def _connect_resume_websocket(
    ...
    allowed_headers = {
        "X-OMB-Launch-ID",
        "X-OMB-Peer-Lineage-ID",
        "X-OMB-Peer-Generation",
    }
...
class ControllerConnection:
    ...
    def _resume_headers(self) -> dict[str, str]:
        headers = {"X-OMB-Launch-ID": self.launch_id}
        if self.authority == "peer":
            if self.lineage_id is None:
                raise ControllerConnectionError("peer lineage is unavailable")
            headers.update({
                "X-OMB-Peer-Lineage-ID": self.lineage_id,
                "X-OMB-Peer-Generation": str(self.generation),
            })
        return headers
    ...
    def start_reader(self) -> None:
        ...
        self._reader_thread = threading.Thread(
            target=receive_loop,
            name=f"omb-controller-{self.authority}-receiver",
            daemon=True,
        )
