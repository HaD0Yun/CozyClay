...
import oh_my_blender.stage_scene as stage_scene_module
from oh_my_blender import hand_shapes, motion_retarget

from oh_my_blender.hand_shapes import CANONICAL_ROLE_ORDER, LIBRARY_VERSION
from oh_my_blender.manifest import (
    _animation_snapshot,
    animation_fcurves,
    extract_scene_manifest_v2,
    extract_scene_snapshot,
)
from oh_my_blender.connection import DurableCommitReconciliationRequired
from oh_my_blender.snapshot import UNSUPPORTED_FCURVE_FEATURE
...
def _independent_dense_channels(ybot, dense_rotations, dense_joints, start_frame):
    ...
    inventory = hand_shapes.validate_rig_bones(
        ybot.get("omb.character_type"), (bone.name for bone in bones)
    )
...
def _benchmark_writer_order():
    value = os.environ.get("OMB_BENCHMARK_WRITER_ORDER", "bulk_first")
    if value == "bulk_first":
        return ("bulk", "legacy")
    if value == "legacy_first":
        return ("legacy", "bulk")
    raise RuntimeError(
        "OMB_BENCHMARK_WRITER_ORDER must be bulk_first or legacy_first"
    )
...
def _install_motion_receipt_logger():
    logger = logging.getLogger("oh_my_blender.motion_keyframes")
...
def main():
    ...
    camera = next(o for o in bpy.data.objects if o.get("omb.entity_id") == CAMERA_ID)
    ...
    with tempfile.TemporaryDirectory() as project_directory:
        motions = pathlib.Path(project_directory) / ".omb" / "motions"
        ...
        results["relaxedFingerCompletion"] = (
            ybot.animation_data.action["omb.hand_pose"] == "relaxed"
        ...
        results["openFingerOverride"] = (
            ybot.animation_data.action["omb.hand_pose"] == "open"
            and ybot.pose.bones[
        ...
        results["asymmetricHandShapes"] = (
            asymmetric_action["omb.hand_shape_left"] == "point"
            and asymmetric_action["omb.hand_shape_right"] == "fist"
            and asymmetric_action["omb.hand_shape_library"] == 1
            and asymmetric_action.get("omb.hand_pose") is None
            and ybot.pose.bones[
                "mixamorig:LeftHandIndex3"
            ].rotation_quaternion.angle < 1e-6
            and ybot.pose.bones[
        ...
        matching_dense_receipts = [
            ...
            if receipt.get("schema") == "omb.stage_scene_motion.v2"
        ...
        legacy_action, legacy_slot, legacy_curves, _legacy_parity_ms = (
            _write_legacy_benchmark_action(
                ybot, channels, "OMB Legacy Dense Parity"
            )
        )
            ...
            if writer == "bulk":
                action, _slot, curves, elapsed_ms = (
                    _write_bulk_benchmark_action(
                        ybot, channels, "OMB Bulk Benchmark"
                    )
                )
                ...
                action, _slot, curves, elapsed_ms = (
                    _write_legacy_benchmark_action(
                        ybot, channels, "OMB Legacy Benchmark"
                    )
                )
        ...
        results["denseWriterTemporaryActionsRemoved"] = not any(
            action.name.startswith(("OMB Bulk ", "OMB Legacy "))
