...
oh_my_blender older than the repo (METHOD_NOT_SUPPORTED / unsupported-op
...
project_dir = Path(tempfile.mkdtemp(prefix="omb-stale-reload-"))
os.environ["OMB_PROJECT_DIR"] = str(project_dir)
os.environ["OMB_REPO"] = str(REPOSITORY_ROOT)

try:
    import oh_my_blender

    # A live Blender session has the add-on registered before omb re-attaches.
    ...
    results["helloReportsRepoVersion"] = (
        f"omb.addon_version={repo_version}" in hello["capabilities"]
    )
    ...
    lines = (project_dir / ".omb-blender.pid").read_text(encoding="utf-8").splitlines()
...
print("OMB_STALE_RELOAD_RESULTS=" + json.dumps(results), flush=True)
