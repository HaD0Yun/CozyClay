...
def main() -> None:
    ...
    try:
        ...
        live_camera_manifest = extract_scene_manifest_v3()
        ...
        def cancel_after_first_chunk() -> None:
            if first_chunk_sent.wait(timeout=90):
                send_json({"type": "cancel", "id": cancel_id})

        threading.Thread(target=cancel_after_first_chunk, daemon=True).start()
        cancel_response = receive(connection, cancel_responses, timeout=120)
        ...
        print("OMB_CONNECTED_COMPOSITION_RESULTS=" + json.dumps({
            ...
            "cancelChunkSent": first_chunk_sent.is_set(),
            "cancelCode": cancel_response.get("code"),
        }, separators=(",", ":")))
    finally:
