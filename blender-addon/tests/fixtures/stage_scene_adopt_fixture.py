...
def foreign_mesh_object(name, entity_id, mesh=None):
    ...
    scene_object["omb.entity_id"] = entity_id
...
def main():
    ...
    cube_foreign_before = default_cube.get("omb.owned_project_id") is None
    ...
    sphere_owned = foreign_sphere.get("omb.owned_project_id") == PROJECT_ID
    apply_stage_scene_transaction(
        plan("a" * 64, [{
            "op": "transform_entity",
    ...
    sphere_still_owned = foreign_sphere.get("omb.owned_project_id") == PROJECT_ID
    ...
    shared_unstamped = shared_a.get("omb.owned_project_id") is None

    # Objects owned by another OMB project stay fenced.
    other_owned = foreign_mesh_object("Other Project Object", OTHER_OWNED_ID)
    other_owned["omb.owned_project_id"] = OTHER_PROJECT_ID
    other_code, other_rollback, _other_commit = attempt_adopt(OTHER_OWNED_ID)
    other_owner_kept = other_owned.get("omb.owned_project_id") == OTHER_PROJECT_ID
    ...
    def fail_commit(_candidate):
        ...
        stamped_before_commit = (
            rollback_target.get("omb.owned_project_id") == PROJECT_ID
        )
    ...
    try:
        ...
        pass
    rollback_unstamped = rollback_target.get("omb.owned_project_id") is None
    ...
    print("OMB_STAGE_SCENE_ADOPT_RESULTS=" + json.dumps(results, sort_keys=True))
