...
os.environ["XDG_RUNTIME_DIR"] = tempfile.mkdtemp(prefix="omb-evidence-test-")

from apply_camera_plan_fixture import PROJECT_ID, Connection, setup_scene
from oh_my_blender import fixture_registry, project_store
from oh_my_blender.camera_plan import apply_camera_plan_transaction
from oh_my_blender.directing_evidence import (
    produce_directing_evidence,
    runtime_producer,
)
from oh_my_blender.fixture_registry import load_authorized_fixture
...
# substrate manifest.
PROJECT_DIR = Path(tempfile.mkdtemp(prefix="omb-evidence-project-"))
...
def main() -> None:
    ...
    camera = bpy.data.objects.get("OMB Camera")
    keyed_frames = sorted({
    ...
    child_camera = bpy.data.objects.get("OMB Camera")
    ...
    print(
        "OMB_DIRECTING_EVIDENCE_RESULTS="
