...
from oh_my_blender.handshake import (
...
from oh_my_blender.stage_scene import _OPERATION_KEYS
...
class HandshakeTests(unittest.TestCase):
    ...
    def test_hello_reports_addon_version_method_and_op_surface(self):
        ...
        self.assertEqual(version_entries, ["omb.addon_version=0.2.0"])
        self.assertEqual(
            [entry for entry in hello["capabilities"] if not entry.startswith("omb.")],
            ["mutation_bridge_v2", "scene_manifest_v3", "transaction_commit_v2"],
        )
        self.assertEqual(
            hello["capabilities"][3:],
