...
class SceneManifestV3Tests(unittest.TestCase):
    def v3_parts(self):
        value = parts()
    ...
    def test_v3_hashes_exact_stage_state_and_real_child_revision(self):
        manifest = build_scene_manifest_v3(**self.v3_parts())
    ...
    def test_v3_stage_references_and_area_size_are_closed(self):
        ...
        with self.assertRaises(INVALID_MANIFEST_REFERENCE):
            build_scene_manifest_v3(**missing)

        bad_area = self.v3_parts()
        bad_area["lights"][0]["lightType"] = "AREA"
        with self.assertRaises(Exception):
            build_scene_manifest_v3(**bad_area)

        valid_area = self.v3_parts()
        valid_area["lights"][0].update(lightType="AREA", areaSize=3.0)
        self.assertEqual(build_scene_manifest_v3(**valid_area)["lights"][0]["objectId"], LIGHT_OBJECT)
