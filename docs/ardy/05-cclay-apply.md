...
- `AGENTS.md:135` documents the invariant "The director may mutate any entity except one stamped `cclay.locked_by_human`" with `execute_blender_python` explicitly exempt — that is policy text; no code path found enforces it against typed ops.
...
- "ARDY Core is 20 fps" is a docstring claim (`stage_scene.py:1350-1351`), consistent with fixtures/tests (`ardy_rig_scaffold.py:126`, `test_cinematic_camera_integration.py:72`) and with the wrapper reading `model.motion_rep.fps` (`cclay_constrained_generate.py:975`; `remote:~/ardy/scripts/generate.py:189`), but the model's actual default fps value was not verified on the GPU box (the `motion_rep` package has no literal `fps = 20` I could find). The runtime contract only bounds fps to 1..240 (`motion_retarget.py:55`, `motion_archive.py:305-310`, `ardy-archive-service.ts:291`).
- `cclay.locked_by_human` enforcement: verified absent by repo-wide text search, but no runtime test was executed; if a host-side check exists outside this repo (e.g. in the director session's tool gating), it was not found here.
