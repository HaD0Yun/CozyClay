...
- The director may mutate any entity it owns. Enforcement is by ownership alone: `_require_owned_entity` admits an entity only when `cclay.owned_project_id` equals the current project (`blender-addon/cclay/stage_scene.py:280-317`). At ownership-inversion cutover the migration also stamped `cclay.locked_by_human` on foreign entities, but that property is written and never read, so it carries no authority and MUST NOT be described as a lock; the set it marks is exactly the set the ownership check already rejects. `execute_blender_python` is explicitly exempt from entity ownership enforcement because arbitrary Python cannot be statically bounded. `adopt_entity` remains available for explicit-claim semantics.
