...
// resolvable AND OMB_LIVE_ACCEPTANCE=1 is set, so `npm test` stays hermetic.
//
// Re-run:
//   cd apps/omb-extension && OMB_LIVE_ACCEPTANCE=1 node --import tsx --test test/live-acceptance.test.ts
//
// Evidence: /tmp/omb-e2e-artifacts/ (results.json, revision-chain.json,
...
const ARTIFACT_DIR = process.env.OMB_E2E_ARTIFACTS ?? "/tmp/omb-e2e-artifacts";
...
const LIVE = process.env.OMB_LIVE_ACCEPTANCE === "1" && BLENDER !== undefined;
...
test(
	...
	{ skip: LIVE ? false : "requires Blender and OMB_LIVE_ACCEPTANCE=1", timeout: 900_000 },
	async (t) => {
		...
		const ombDir = path.join(projectDir, ".omb");
		...
		let blender: ChildProcess | undefined = spawn(
			...
			[
				...
				"--python",
				path.join(REPO_ROOT, "apps/omb-extension/test/live-acceptance-sidecar.py"),
				"--",
				"--omb-project-dir",
				projectDir,
				"--omb-repo",
			...
			{ env: { ...process.env, OMB_WATCH_MS: "0" }, stdio: ["ignore", blenderLog, blenderLog], detached: true },
		);
		...
		const commandDir = path.join(projectDir, ".omb-e2e");
		...
		try {
			...
			await call("wait for .omb/project.json", async () => {
			...
			await scenario("S1", "attach handshake accepts the repo addon surface", async () => {
				...
				const surface = (await blenderExec(
					...
					[
						...
						"import oh_my_blender.connection as _connection",
						"from oh_my_blender.handshake import build_hello",
						"_conn = _connection._active_connection",
						"result = {",
						'    "negotiated": sorted(_conn.capabilities),',
						'    "hello_capabilities": build_hello(',
						'        bpy.context.scene["omb.project_id"], oh_my_blender.ADDON_VERSION, bpy.app.version_string',
						'    )["capabilities"],',
						'    "addon_version": oh_my_blender.ADDON_VERSION,',
				...
				assert.ok(surface.hello_capabilities.includes(`omb.addon_version=${REPO_ADDON_VERSION}`));
			...
			await scenario("S4", "produce_directing_evidence digest authorizes apply_camera_plan", async () => {
				...
				const evidenceDirectory = (await blenderExec(
					...
					[
						"import bpy",
						"from oh_my_blender.directing_evidence import runtime_evidence_directory",
						'result = str(runtime_evidence_directory(bpy.context.scene["omb.project_id"]))',
					].join("\n"),
				)) as string;
				...
				const camera = entityByName(after.snapshot, "OMB Camera");
				assert.ok(camera?.entityId, "OMB Camera exists in the post-apply snapshot with an entity id");
				const detail = (await call("inspect_entity OMB Camera animation", () =>
			...
			// ---------------- S2 adopt + delete a pre-existing non-OMB cube ----------------
			await scenario("S2", "foreign cube: inspect shows it, adopt+delete removes it", async () => {
				const seeded = (await blenderExec(
					...
					[
						...
						"assert obj.get('omb.owned_project_id') is None",
						"assert obj.get('omb.entity_id') is None",
						"# Entity id assignment is the repo's canonical repair path; ownership stays foreign.",
						"bpy.ops.omb.repair_ids()",
						"result = {",
						'    "name": obj.name,',
						'    "entity_id": obj.get("omb.entity_id"),',
						'    "owned_project_id": obj.get("omb.owned_project_id"),',
						"}",
					].join("\n"),
				)) as { name: string; entity_id: string; owned_project_id: unknown };
				assert.match(seeded.entity_id, /^[0-9a-f-]{36}$/);
				assert.equal(seeded.owned_project_id, null, "seeded cube carries no OMB ownership");
			...
			await scenario("S8", "render_qa_frames: model sees <64KB JPEG, PNG artifact streamed", async () => {
				...
				assert.equal(frame.uri, `omb-artifact://sha256/${frame.sha256}`);
			...
			await scenario("S6", "read_image accepts a PNG under the real $TMPDIR", async () => {
				...
				const imagePath = path.join(tmpRoot, `omb-e2e-read-image-${process.pid}.png`);
