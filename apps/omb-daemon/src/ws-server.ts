...
export class WebSocketConnection extends EventEmitter {
	...
	constructor(readonly socket: Socket) { super(); socket.on("data", b => this.read(b)); socket.on("close", () => this.emit("disconnect")); }
	...
	private frame(opcode: number, payload: Buffer): void { if (this.socket.destroyed) return; let h: Buffer; if (payload.length < 126) h = Buffer.from([0x80 | opcode, payload.length]); else if (payload.length <= 65535) { h=Buffer.alloc(4); h[0]=0x80|opcode; h[1]=126; h.writeUInt16BE(payload.length,2); } else { h=Buffer.alloc(10); h[0]=0x80|opcode; h[1]=127; h.writeBigUInt64BE(BigInt(payload.length),2); } this.socket.write(Buffer.concat([h,payload])); }
...
export function acceptUpgrade(req: IncomingMessage, socket: Socket, token: BearerToken, port: number, alreadyAccepted: boolean): WebSocketConnection | undefined {
