...
export async function start(options:DaemonOptions):Promise<Daemon>{
	...
	function run(ws:WebSocketConnection){let hello=false, drainPromise:Promise<void>|undefined;const activeHandlers=new Set<Promise<void>>();const helloTimer=setTimeout(()=>{if(!hello)ws.close(1008,"hello timeout");},options.helloTimeoutMs??3000);const state=new SessionState(clock,r=>void finishCancellation(r));
		...
		async function execute(request:Request){
			if(draining)return ws.sendText(error(request.id,"SHUTTING_DOWN","daemon is shutting down",true));
		...
		function drain(cause:"SHUTDOWN"|"DISCONNECT",acknowledge:boolean):Promise<void>{if(drainPromise)return drainPromise;draining=true;server.close();const r=state.current;if(r)state.cancel(r.id,cause);drainPromise=(async()=>{if(activeHandlers.size){let timer:ReturnType<typeof setTimeout>|undefined;const bounded=new Promise<void>(resolve=>{timer=setTimeout(resolve,5000);});await Promise.race([Promise.allSettled(Array.from(activeHandlers)),bounded]);clearTimeout(timer);}if(acknowledge&&!ws.socket.destroyed)ws.sendText({type:"shutdown_ack"});if(!ws.socket.destroyed)ws.close(1000);clearTimeout(idle);token.zero();try{await closeServer();}catch(e){(options.stderr??(line=>process.stderr.write(line+"\n")))(`daemon cleanup failed: ${e instanceof Error?e.message:String(e)}`);}resolveStopped();})();return drainPromise;}
	}
