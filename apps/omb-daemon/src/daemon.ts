...
export type Handler=(params:Record<string,unknown>, context:{signal:AbortSignal;request:Request})=>Promise<HandlerResult>;
export type DaemonOptions={port:number;clock?:Clock;handlers:Record<string,Handler>;stdout?:(line:string)=>void;stderr?:(line:string)=>void};
...
export async function start(options:DaemonOptions):Promise<Daemon>{
	...
	function run(ws:WebSocketConnection){let hello=false;const helloTimer=setTimeout(()=>{if(!hello)ws.close(1008,"hello timeout");},3000);const state=new SessionState(clock,r=>void finishCancellation(r));
		const resetIdle=()=>{clearTimeout(idle);idle=setTimeout(()=>ws.close(1000,"idle"),60000);};resetIdle();
		...
		async function message(text:string){let raw:any;try{raw=JSON.parse(text);}catch{state.consumeToken();return;}if(!hello){try{const h=parseHello(raw);if(nonces.has(h.client_nonce))return ws.close(1008,"nonce reused");nonces.add(h.client_nonce);hello=true;clearTimeout(helloTimer);ws.sendText({type:"hello_ack",protocol:1,daemon_version:"0.1.0",launch_id:launchId,session_id:randomUUID(),server_nonce:randomNonce(),capabilities:["inspect_project"]});return;}catch{return ws.close(1008,"invalid hello");}}
			if(raw?.type==="ping"){ws.sendText({type:"pong",nonce:raw.nonce});return;}if(raw?.type==="cancel"){const status=state.cancel(raw.id);ws.sendText({type:"cancel_ack",id:raw.id,status});return;}if(raw?.type==="shutdown"){await shutdown();return;}if(raw?.type!=="request"){try{parseClientMessage(raw);}catch{state.consumeToken();}return;}
			...
			try{const out=await handler(request.params,{signal:r.controller.signal,request});if(state.complete(r)){state.terminal(r);ws.sendText({type:"response",id:r.id,...out});}}catch(e){if(r.phase==="running"&&state.complete(r)){state.terminal(r);ws.sendText(error(r.id,"HANDLER_ERROR",e instanceof Error?e.message:"handler failed",false));}}
		}
		...
		async function shutdown(){if(draining)return;draining=true;server.close();const r=state.current;if(r)state.cancel(r.id,"SHUTDOWN");await Promise.race([new Promise(res=>setTimeout(res,8000)),Promise.resolve()]);ws.sendText({type:"shutdown_ack"});ws.close(1000);}
	}
