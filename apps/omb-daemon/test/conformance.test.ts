...
test("silent protocol-v2 bridge is force-destroyed after the close grace",async()=>{const {d,c}=await readyV2({idleTimeoutMs:20});try{c.socket.pause();await new Promise(r=>setTimeout(r,1100));assert.equal(c.socket.destroyed,false);const closed=new Promise<void>(r=>c.socket.once("close",()=>r()));c.socket.resume();await closed;assert.equal(c.socket.destroyed,true);assert.equal(c.socket.closed,true);}finally{c.socket.destroy();await d.close();}});
