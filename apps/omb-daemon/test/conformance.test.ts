...
test("silent protocol-v2 bridge is force-destroyed after the close grace",async()=>{let forced=0,resolveForced!:()=>void;const forceDestroyed=new Promise<void>(r=>resolveForced=r);const {d,c}=await readyV2({idleTimeoutMs:20,testHooks:{onIdleForceDestroy:()=>{forced++;resolveForced();}}});try{c.socket.pause();await Promise.race([forceDestroyed,new Promise((_,reject)=>setTimeout(()=>reject(new Error("force-destroy timeout")),1200))]);assert.equal(forced,1);assert.equal(c.socket.destroyed,false);const closed=new Promise<void>(r=>c.socket.once("close",()=>r()));c.socket.destroy();await closed;assert.equal(c.socket.destroyed,true);assert.equal(c.socket.closed,true);}finally{c.socket.destroy();await d.close();}});
