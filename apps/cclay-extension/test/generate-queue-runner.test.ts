...
// ever looks at the directory the add-on writes to. For the generate queue
...
import { startGenerateQueueRunner } from "../src/generate-queue-runner.ts";

const REVISION = "a".repeat(64);
const ENTITY = "3f2504e0-4f89-41d3-9a0c-0305e82c3301";
const REQUEST_ID = "0123456789abcdef0123456789abcdef";

function aRequest(overrides: Partial<ArdyGenerateRequestV1> = {}): ArdyGenerateRequestV1 {
	return {
		schema_version: 1,
...
printf '%s\\n' "$@" > "$ARGV_LOG"
echo '{"motion_id":"generated-clip","frames":100,"fps":20,"duration_s":5,"continuity":{"mean_jump_m":0,"max_jump_m":0.01,"max_jump_frame":0}}'
`;
...
describe("generate queue runner", () => {
	let project: string;
	let wrapper: string;

	beforeEach(async () => {
		project = await mkdtemp(join(tmpdir(), "cclay-generate-runner-"));
	...
	async function publish(request: object) {
		const directory = join(project, ".cclay", "generate-requests");
	...
	it("consumes a request the add-on published and applies the clip", async () => {
		...
		const runner = startGenerateQueueRunner({
			cwd: project,
		...
		assert.deepEqual(applied, [{ motionId: "generated-clip", revision: REVISION }]);
		assert.deepEqual(archiveEvents, ["commit:generated-clip", "apply"]);
		const outcome = JSON.parse(
			await readFile(join(project, ".cclay", "generate-outcomes", `${REQUEST_ID}.json`), "utf8"),
		);
		assert.equal(outcome.status, "succeeded");
		assert.equal(outcome.result.motion_id, "generated-clip");
		assert.deepEqual(await readdir(join(project, ".cclay", "generate-requests")), []);
	});
	...
	it("submits a model request through the same durable queue and returns its outcome", async () => {
		...
		const outcome = await runner.generate(aRequest());
		await runner.stop();
