...
- Use `capture_viewport` for fast iterative checks. Inspect up to 3 useful views: an establishing view, a side view that reveals depth/contact, and a close view of the subject or interaction; once the numeric checks pass, one confirming view suffices for non-contact relations. A declared `support`/`contact`/`seat`/`lean`/`grasp` relation always needs at least two views, including one that reveals the contact/support gap, before it can be approved — never approve actor-object contact from a single camera angle. Add another view only when something is occluded.
