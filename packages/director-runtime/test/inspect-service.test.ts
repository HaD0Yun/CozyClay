...
async function setup(responses: Parameters<ReturnType<typeof registerFauxProvider>["setResponses"]>[0]) {
	const faux = registerFauxProvider(); unregister.push(faux.unregister); faux.setResponses(responses);
	const credentials = new InMemoryCredentialStore();
	await credentials.modify(faux.getModel().provider, async () => ({ type: "api_key", key: "faux-key" }));
	const modelRuntime = await ModelRuntime.create({ credentials, modelsPath: null });
	const model = faux.getModel(); modelRuntime.registerProvider(model.provider, { baseUrl: model.baseUrl, api: faux.api, models: faux.models });
	return { model, modelRuntime };
}

it("validates a snapshot and returns the inspected canonical manifest", async () => {
	const runtime = await setup([fauxAssistantMessage(fauxToolCall("inspect_project", {}), { stopReason: "toolUse" }), fauxAssistantMessage("done")]);
	const snapshot = JSON.parse(await readFile(new URL("../../blender-protocol/test/fixtures/blender-exported-snapshot.json", import.meta.url), "utf8"));
	...
	assert.deepEqual(result.result, { revision: canonicalRevision(snapshot), sceneName: snapshot.scene.name, objectNames: snapshot.objects.map((object: {name:string}) => object.name) });
});
