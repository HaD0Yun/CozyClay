...
const DIRECTOR_PROMPT_CONTRACT_CORE =
	...
	"Use ardy_regenerate only to constrain an existing base motion through the typed host queue; it commits the validated archive and applies the result through the durable mutation path. Unconstrained text-to-motion generation is not exposed as a director tool. Do not present raw Python as a trusted ARDY path: typed APIs are correctness boundaries for well-behaved callers, not security isolation. After significant visual changes, verify with economical multi-view and motion QA; never approve from one angle.";
...
6. Use ardy_regenerate only to constrain an existing base motion through the typed host queue. The host commits the validated archive and applies the result through the durable mutation path. Unconstrained text-to-motion generation is not exposed as a director tool. Do not present raw Python as a trusted ARDY path. Typed APIs are correctness boundaries for well-behaved callers only, never security isolation.
