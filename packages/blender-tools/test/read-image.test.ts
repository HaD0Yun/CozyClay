...
describe("read_image", () => {
	const projectDir = mkdtempSync(path.join(tmpdir(), "omb-read-image-project-"));
	const scratchDir = mkdtempSync(path.join(tmpdir(), "omb-read-image-scratch-"));
	...
	it("reads a /tmp-prefixed path even when /tmp is a symlink (macOS /private/tmp)", { skip: !POSIX }, async () => {
		const imagePath = `/tmp/omb-read-image-${process.pid}-${Date.now()}.png`;
		writeFileSync(imagePath, PNG);
	...
	it("rejects a path outside the project, home, and temp roots", async () => {
		await assert.rejects(
			execute(path.join(path.parse(projectDir).root, "omb-definitely-not-allowed", "x.png")),
