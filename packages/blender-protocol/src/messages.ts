...
// Addon-reported staleness surface: `omb.addon_version=<semver>`,
// `omb.method.<bridge_method>`, `omb.op.<stage_scene_op>`. These ride the
// hello capabilities array without widening the negotiated core set.
const NAMESPACED_CAPABILITY_PATTERN = "^omb\\.[A-Za-z0-9_.=-]+$";
...
// The addon->daemon hello additionally carries namespaced omb.* surface
...
export function negotiateMutationBridge(helloInput: unknown, helloAckInput: unknown): MutationBridgeSession {
	...
	// The hello capability shape tolerates namespaced omb.* surface entries, so
