...
describe("Architecture §4 protocol v2 mutation bridge", () => {
	...
	it("tolerates namespaced omb.* surface capabilities in hello but not in hello_ack", () => {
		const surfaced = {
			...v2Hello,
			capabilities: [
				MUTATION_CAPABILITY,
				STAGE_CAPABILITY,
				"omb.addon_version=0.2.0",
				"omb.method.inspect_project",
				"omb.op.transform_entity",
			],
		};
		...
		assert.throws(
			() =>
				negotiateMutationBridge(
					{ ...v2Hello, capabilities: [STAGE_CAPABILITY, "omb.addon_version=0.2.0"] },
					v2HelloAck,
				),
			/mutation_bridge_v2/,
		);
		// The hello_ack negotiated set stays a closed tuple: no omb.* riders.
		assert.throws(() =>
			negotiateMutationBridge(surfaced, {
				...v2HelloAck,
				capabilities: [MUTATION_CAPABILITY, STAGE_CAPABILITY, "omb.addon_version=0.2.0"],
			}),
		);
	});
